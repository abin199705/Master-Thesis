# -*- coding: utf-8 -*-
"""
Created on Fri Jun  4 20:24:01 2021
...
x_train[:, 24:40, 24:40, :] = x_train[:, 24:40, 24:40, :]/2
x_train = x_train*2
...
# AE模型架構宣告
input_img_1 = Input(shape=(64, 64, 3))
...
conv_13T = Conv2DTranspose(3,(3, 3), padding='same')(conv_12T)
...
autoencoder_1.compile(optimizer='adam', loss=SSIMLoss, metrics=['accuracy'])
autoencoder_1.summary()



# 將AE模型架構存成圖片檔
plot_model(autoencoder_1, to_file='./save/VGG_autoencoder_SSIM_MASK_out.png', show_shapes=True)


autoencoder_1.fit(x_train, x_train, nb_epoch=5000, batch_size=128, shuffle=True)

decoded_imgs_1 = autoencoder_1.predict(x_train)
np.save('./save/VGG_decoded_imgs_1_SSIM_MASK_RGB_out', decoded_imgs_1)
AE1_code = encoder.predict(x_train)
np.save('./save/VGG_AE1_code_SSIM_MASK_RGB_out', AE1_code)

encoder.save("./save/VGG_encoder_model_1_SSIM_MASK_RGB_out.h5")
autoencoder_1.save("./save/VGG_autoencoder_1_SSIM_MASK_RGB_out.h5")
...
#plt.imshow(np.squeeze(decoded_imgs_1[1]), cmap=plt.cm.gray_r)
plt.imshow(np.squeeze(decoded_imgs_1[1]/255))
plt.show()
# plt.imshow(np.squeeze(x_train[9]/255), cmap=plt.cm.gray_r)
