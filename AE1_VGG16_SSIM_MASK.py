...
autoencoder_1.fit(x_train, x_train, nb_epoch=10000, batch_size=128, shuffle=True)

decoded_imgs_1 = autoencoder_1.predict(x_train)
np.save('./save/VGG_decoded_imgs_1_SSIM_MASK', decoded_imgs_1)
AE1_code = encoder.predict(x_train)
np.save('./save/VGG_AE1_code_SSIM_MASK', AE1_code)

encoder.save("./save/VGG_encoder_model_1_SSIM_MASK.h5")
autoencoder_1.save("./save/VGG_autoencoder_1_SSIM_MASK.h5")
...
plt.imshow(np.squeeze(x_train[1]), cmap=plt.cm.gray_r)
plt.show()
