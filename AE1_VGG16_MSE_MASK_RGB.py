# -*- coding: utf-8 -*-
"""
Created on Sun May 23 16:44:36 2021
...
# 將AE模型架構存成圖片檔
plot_model(autoencoder_1, to_file='./save/VGG_autoencoder_MSE_MASK.png', show_shapes=True)


autoencoder_1.fit(x_train, x_train, nb_epoch=10000, batch_size=128, shuffle=True)

decoded_imgs_1 = autoencoder_1.predict(x_train)
np.save('./save/VGG_decoded_imgs_1_MSE_MASK_RGB', decoded_imgs_1)
AE1_code = encoder.predict(x_train)
np.save('./save/VGG_AE1_code_MSE_MASK_RGB', AE1_code)

encoder.save("./save/VGG_encoder_model_1_MSE_MASK_RGB.h5")
autoencoder_1.save("./save/VGG_autoencoder_1_MSE_MASK_RGB.h5")
...
plt.show()
plt.imshow(np.squeeze(x_train[1]/255))
plt.show()
