...
# 將AE模型架構存成圖片檔
plot_model(autoencoder_1, to_file='./save/VGG_autoencoder_MSE_RGB.png', show_shapes=True)


autoencoder_1.fit(x_train, x_train, nb_epoch=5000, batch_size=128, shuffle=True)

decoded_imgs_1 = autoencoder_1.predict(x_train)
np.save('./save/VGG_decoded_imgs_1_MSE_RGB', decoded_imgs_1)
AE1_code = encoder.predict(x_train)
np.save('./save/VGG_AE1_code_MSE_RGB', AE1_code)

encoder.save("./save/VGG_encoder_model_1_MSE_RGB.h5")
autoencoder_1.save("./save/VGG_autoencoder_1_MSE_RGB.h5")
...
plt.show()
plt.imshow(np.squeeze(x_train[0]/255))
plt.show()